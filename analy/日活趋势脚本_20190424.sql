...
select '各app日活数据' model,
 ...
 where pt_days>='20190401' and pt_days<'20190423'
group by pt_days,
         appname;
...
from 
    (select pt_days,
        ...
        (select  pt_days,
                ...
                max(case when (url_host='ip2.kugou.com' and instr(urltag,'vip_type=6')>0 ) or (url_host='newsongretry.kugou.com' and instr(urltag,'vip_type=6')>0 )
                 or (url_host='ip.kugou.com' and instr(urltag,'vip_type=6')>0 ) or (url_host='ads.service.kugou.com' and instr(urltag,'isVip=1')>0 ) then 1 else 0 end) tag10
            from  user_action_tag_d
            where pt_days>='20190401' and pt_days<'20190423'  
...
from 
    (select pt_days,
        ...
        (select  pt_days,
                ...
                max(case when url_host ='mp.ximalaya.com' and instr(urltag,'ktv_room')>0 then 1 else 0 end) tag5,
                max(case when url_host ='searchwsa.ximalaya.com' and instr(urltag,'liveroom')>0 then 1 else 0 end) tag6,
...
from 
    (select 
      ...
      (select pt_days,
            ...
            max(case when instr(urltag,'video/play')>0  or instr(urltag,'action=GetPlayInfo')>0 or instr(urltag,'article/information')>0 or instr(urltag,'answer/detail')>0 then 1 else 0 end) tag9
          from  user_action_tag_d
          where  pt_days>='20190401' and pt_days<'20190423'
...
from user_action_tag_d where  pt_days in ('20190420','20190421') and instr(url_host,'dingtalk.com')>0  group by pt_days,url_host;

--钉钉总域名统计
select '钉钉总域名统计' model,pt_days,count(*) cnt,count(distinct device_number) dau
from user_action_tag_d where  pt_days>='20190401' and pt_days<'20190423' and instr(url_host,'dingtalk.com')>0 group by pt_days;

--钉钉分域名统计2
select '钉钉分域名统计' model,pt_days,url_host,count(*) cnt,count(distinct device_number) dau
from user_action_tag_d where  pt_days>='20190401' and pt_days<'20190423' and url_host in 
...
from user_action_tag_d where  pt_days in ('20190420','20190421') and instr(urltag,'antfortune.com')>0  group by pt_days,url_host; 
...
select  '支付宝各模块统计' model,
        ...
        sum(case when tagg39>0 then 1 else 0 end)  uv39,
  ...
  from 
    (select   pt_days,
              device_number,
              max(case when tag_cf=1 then 1 else 0 end) tagg0,
              sum(case when tag1=1 then 1 else 0 end)    tagg1,
              sum(case when tag2=1 then 1 else 0 end)    tagg2,
              sum(case when tag3=1 then 1 else 0 end)    tagg3,
              ...
              sum(case when tag20=1  and tag_login=1    then 1 else 0 end)    tagg20,
              ...
              sum(case when tag30=1  and tag_login=1    then 1 else 0 end)    tagg30,
              ...
              sum(case when tag39=1  and tag_login=1    then 1 else 0 end)    tagg39,
    ...
    from ( select pt_days,
                  device_number,
                  hh,
                  max(case when instr(urltag,'antfortune.com')>0 then 1 else 0 end) tag_cf,
                  ...
                  max(case when instr(url_host,'zmxy.com.cn')>0 then 1 else 0 end) tag20,
                  ...
                  max(case when url_host='orders.kfc.com.cn' then 1 else 0 end) tag29,
                  max(case when url_host='imgorder.kfc.com.cn' then 1 else 0 end) tag30,
                  ...
                  max(case when url_host='fundcaifuhao.antfortune.com' then 1 else 0 end) tag39,
            ...
            where  pt_days>='20190401' and pt_days<'20190423'
              and  (url_host in ('loggw.alipay.com','mdap.alipay.com','mcgw.alipay.com','mobilegw.alipay.com','mfbizweb.19ego.cn','api.mgzf.com',
                            ...
                            'fundcaifuhao.antfortune.com','stockcaifuhao.antfortune.com','advcaifuhao.antfortune.com','pecaifuhao.antfortune.com',
                    ...
                    or instr(urltag,'antfortune.com')>0)
                    group by pt_days,
                          device_number,
                          hh) a
                group by  pt_days,
...
--各个host的数据统计
select url_host,count(*) cnt,count(distinct device_number) dau
from user_action_tag_d where  pt_days='20190420'  group by url_host;


--各个host,urltag组合的数据统计
select url_host,urltag,count(*) cnt,count(distinct device_number) dau
   from user_action_tag_d 
 where pt_days='20190420' 
