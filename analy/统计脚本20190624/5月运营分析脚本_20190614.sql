...
(select device_number,
     ...
     from 
       tmp_user_app_yy_t3
    where pt_days>='20190601' and pt_days<'20190701' and rn=1
      and appname in ('tencent_news','toutiao','kuaibao','wangyinews','sina_news','sohu_news','ifeng') 
...
(select device_number,
     ...
     from 
       tmp_user_app_yy_t3
    where pt_days>='20190301' and pt_days<'20190401' and rn1=1
  group by device_number,
           appname) b 
  where exists 
    (select * 
...
select
       '各app201903新增-201904,05留存' model,
       b.appname,
       count(distinct b.device_number) uv
 from 
(select device_number,
...
select
       '各app201903新增-201904留存--剔除流量卡' model,
       b.appname,
       count(distinct b.device_number1) uv
 from 
(select device_number1,
...
select
       '各app201903新增-201904,05留存--剔除流量卡' model,
       b.appname,
       count(distinct b.device_number1) uv
 from 
(select device_number1,
...
select
       '各app201904新增-201905留存--剔除流量卡' model,
       b.appname,
       count(distinct b.device_number1) uv
 from 
(select device_number1,
