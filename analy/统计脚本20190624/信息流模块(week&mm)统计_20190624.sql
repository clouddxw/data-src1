...
select '手机百度各模块周统计' model,
        ...
        count(distinct(case when tag13=1 then device_number else null end)) uv12,
        ...
        sum(case when tag13=1 then 1 else 0 end) pv12       
from 
        (select pt_days,
                device_number,
                hh,
                ...
                max(case when url_host in ('feed.baidu.com','mbd.baidu.com','ext.baidu.com') or url_host rlike '^(sp)([0-9]*)(\.baidu.com)$' or url_host rlike '^(vd)([0-9]*)(\.bdstatic.com)$' then 1 else 0 end) tag13
        from  user_action_tag_d
        where pt_days>='20190506' and pt_days<'20190624'  
...
select 'UC各模块周统计' model,
    ...
    from 
        (select pt_days,
                ...
                max(case when url_host in ('image.uczzd.cn','iflow.uczzd.cn','track.uc.cn','img.v.uc.cn') then 1 else 0 end) tag11
            from  user_action_tag_d
            where pt_days>='20190506' and pt_days<'20190624'  
