...
drop table tmp_user_app_yy_t2;
create table tmp_user_app_yy_t2 as
select t.*,
       t1.device_number device_number1
    from 
        tmp_user_app_yy_t2  t
     left join 
        (select device_number 
         from zba_dwa.dwa_v_m_cus_nm_user_info b  
         where b.month_id='201904'
...
(select device_number1,
     ...
     from 
       (select  pt_days,
                appname,
                device_number1,
                row_number() over(partition by appname,device_number1 order by pt_days asc) rn
            from  tmp_user_app_yy_t2
            where appname in ('tencent_news','toutiao','kuaibao','wangyinews','sina_news','sohu_news','ifeng','qq_music','kuwo','kugou','wangyi_music')             
            )  a
where rn=1
