...
select substr(pt_days,0,6) pt_days,
   ...
   where pt_days>='20190301' and pt_days='20190731'
     group by 
       substr(pt_days,0,6),
       device_number,
       device_number1,
...
from 
  (select  pt_days,
          ...
          tarapp,
          srcapp,
          max(case when c.diff>0 and c.diff<=90 then 1 else 0 end) flg
  from 
  (select b.pt_days,
          b.device_number,
          b.device_number1,
...
select
       '各app201905新增-201906,07留存' model,
       'App' appname,
       'uv' uv,
       'uv_剔除流量卡' uv1;
select
  ...
  where exists 
    (select * 
       ...
       (select
            ...
            from  tmp_user_app_yy_m
            where pt_days in ('201906','201907') 
          group by device_number,
                   appname    
            )  b1
        where  b.device_number=b1.device_number
...
from 
    (select 
            ...
            tarapp,
            device_number1,
            max(case when c.diff>=0 and c.diff<89 then 1 else 0 end) flg
    from 
    (select     b.pt_days,
                b.device_number,
                b.appname srcapp,
            ...
            from 
                (select substr(pt_days,0,6) pt_days,
                    ...
                    group BY
                        ...
                        device_number) a) b
        left join 
            (select * from tmp_user_app_yy_m  where appname='ximalaya' and pt_days>='201906') b1 
