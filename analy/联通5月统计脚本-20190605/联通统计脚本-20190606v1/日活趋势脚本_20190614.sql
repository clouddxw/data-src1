...
select '各app日活数据' model,
 ...
 where pt_days>='20190515' and pt_days<'20190610'
group by pt_days,
         appname;
...
select '各app日活数据--剔除流量套餐' model,
   ...
   from 
        (select *
        from  user_action_app_d 
        where pt_days>='20190515' and pt_days<'20190610')  t1
     INNER join 
...
where pt_days>='20190501' and pt_days<'20190601'
...
-- from user_action_tag_d where  pt_days in ('20190513','20190514') and instr(url_host,'dingtalk.com')>0  group by pt_days,url_host;

-- --支付宝财富模块二级域名统计
-- select '支付宝财富模块二级域名统计' model,pt_days,url_host,count(*) cnt,count(distinct device_number) dau
-- from user_action_tag_d where  pt_days in ('20190513','20190514') and instr(url_host,'antfortune.com')>0  group by pt_days,url_host; 

-- --京东域名分布
-- select '京东域名分布' model,pt_days,url_host,count(*) cnt,count(distinct device_number) dau
-- from user_action_tag_d where  pt_days in ('20190513','20190514') and instr(url_host,'.m.jd.com')>0  group by pt_days,url_host; 

-- --饿了么域名分布
-- select '饿了么域名分布' model,pt_days,url_host,count(*) cnt,count(distinct device_number) dau
-- from user_action_tag_d where  pt_days in ('20190513','20190514') and instr(url_host,'.ele.me')>0  group by pt_days,url_host; 

-- --钉钉总域名统计
-- select '钉钉总域名统计' model,pt_days,count(*) cnt,count(distinct device_number) dau
-- from user_action_tag_d where  pt_days>='20190508' and pt_days<'20190518' and instr(url_host,'dingtalk.com')>0 group by pt_days;
...
-- from user_action_tag_d where  pt_days>='20190508' and pt_days<'20190518' and url_host in 
...
-- 'sh.preview.dingtalk.com','dtliving.alicdn.com','vote.dingteam.com')  group by pt_days,url_host order by pt_days,url_host;
...
from 
        ...
        from user_action_tag_d 
        where  pt_days>='20190520' and pt_days<'20190610'
...
from user_action_tag_d where  pt_days='20190531'  group by url_host;
