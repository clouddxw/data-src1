    ...
    public void runToutiaoClickAnalyzer(String ptDays, String dataBase) {
        runSparkJob(new CommonSparkJob(spark, logger, 1, dataBase, UnicomTable.USER_ACTION_CONTEXT_OUT, null, ptDays, null, null){
            ...
            public Dataset<Row> run() throws UnsupportedEncodingException {
                ...
                //获取前两个月的日期
                String ptMonth = DateUtils.getPervMonth(ptDays,"yyyyMMdd",-2);
                ...
                WindowSpec windowSpec = Window.partitionBy(userInfo.col("device_number")).orderBy(relationUserInfo.col("user_id"));

                //tt_con.join(useInfo,tt_con.col("device_number").equalTo(useInfo.col("device_number")))
                Dataset<Row> toutiaoCilck = userInfo.join(relationUserInfo,tt_con.col("device_number").equalTo(relationUserInfo.col("device_number")))
                        .withColumn("rank",row_number().over(windowSpec))
                        .select(relationUserInfo.col("user_id"),userInfo.col("cust_sex"),
                        ...
                        .where("rank = 1")
                        .join(tt_con,tt_con.col("device_number").equalTo(userInfo.col("device_number")),"right")
                        .selectExpr( "binaryconversion(murmurhash(user_id)) as irt",
                                "cust_sex","age_range",
            ...
            runSparkJob(new CommonSparkJob(spark, logger, dropPartitionType, dataBase, UnicomTable.USER_ACTION_TAG_D, null, ptDays, null, provId) {
                ...
                public Dataset<Row> run() throws UnsupportedEncodingException {
                    Dataset<Row> tmpSampleBase = spark.sql("  select device_number," +
                            ...
                            "                when instr(url_host,'.ximalaya.com')>0 then concat_ws('&',regexp_extract(url,'(vipStatus=1)',1),regexp_extract(url,'(vipStatus=2)',1),regexp_extract(url,'(vipStatus=3)',1),regexp_extract(url,'(vipCategory)',1),regexp_extract(url,'(recharge)',1),concat_ws('%',regexp_extract(url,'(positionName=focus)',1),regexp_extract(url,'(categoryId=-2)',1)),regexp_extract(url,'(AggregateRankListTabs)',1),regexp_extract(url,'(daily)',1),regexp_extract(url,'(sceneId=[0-9]+)',1),regexp_extract(url,'(sceneId=)',1),concat_ws('%',regexp_extract(url,'(recommends)',1),regexp_extract(url,'(categoryId=33)',1)),regexp_extract(url,'(topBuzz)',1),concat_ws('%',regexp_extract(url,'(vip)',1),regexp_extract(url,'(channel)',1),regexp_extract(url,'(categoryId=-8)',1)),regexp_extract(url,'(categoryId=[0-9]+)',1)) "+
                            "                when url_host='interface.music.163.com' then concat_ws('&',regexp_extract(url,'(enhance/download)',1),regexp_extract(url,'(comment)',1),regexp_extract(url,'(store)',1)) "+
                            "                when instr(url_host,'.snssdk.com')>0  and url like '%app_name=news_article%' then concat_ws('&',regexp_extract(url,'(app_name=news_article)',1),regexp_extract(url,'(news/feed)',1),regexp_extract(url,'(article/information)',1),regexp_extract(url,'(keyword)',1),regexp_extract(url,'(category=video)',1),regexp_extract(url,'(action=GetPlayInfo)',1),regexp_extract(url,'(category=hotsoon_video)',1),regexp_extract(url,'(category=live)',1),regexp_extract(url,'(video/play)',1),regexp_extract(url,'(videolive)',1),regexp_extract(url,'(get_ugc_video)',1),regexp_extract(url,'(answer/detail)',1),regexp_extract(url,'(video/openapi)',1)) "+
