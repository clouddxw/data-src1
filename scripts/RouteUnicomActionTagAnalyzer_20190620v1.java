        ...
        runSparkJob(new CommonSparkJob(spark, logger, 1, dataBase, UnicomTable.USER_ACTION_CONTEXT_OUT, null, ptDays, null, null){
            ...
            public Dataset<Row> run() throws UnsupportedEncodingException {
                ...
                Dataset<Row> tt_con =  spark.sql( "  select pt_days,"+
                        "         device_number,"+
                        "         case when model='pdd' then concat_ws('$',collect_set(con1)) "+
                        "              else concat_ws('$',collect_set(concat_ws(':',con1,con3))) "+
                        "         end as tt_con, "+
                        "         model  tx_con "+
                        "   from   "+ dataBase + "."+UnicomTable.USER_ACTION_CONTEXT_D +" a"+
                        "    where pt_days = '" + ptDays + "'" +
                        "      and model in ('toutiao_article','pdd','tencent_news_article','tencent_news_video') "+
            ...
            runSparkJob(new CommonSparkJob(spark, logger, dropPartitionType, dataBase, UnicomTable.USER_ACTION_TAG_D, null, ptDays, null, provId) {
                ...
                public Dataset<Row> run() throws UnsupportedEncodingException {
                    Dataset<Row> tmpSampleBase = spark.sql(" select device_number," +
                            ...
                            "                when instr(url_host,'.snssdk.com')>0  and instr(url,'app_name=news_article')>0 then concat_ws('&',regexp_extract(url,'(app_name=news_article)',1),regexp_extract(url,'(news/feed)',1),regexp_extract(url,'(article/information)',1),regexp_extract(url,'(keyword)',1),regexp_extract(url,'(category=video)',1),regexp_extract(url,'(action=GetPlayInfo)',1),regexp_extract(url,'(category=hotsoon_video)',1),regexp_extract(url,'(category=live)',1),regexp_extract(url,'(video/play)',1),regexp_extract(url,'(videolive)',1),regexp_extract(url,'(get_ugc_video)',1),regexp_extract(url,'(answer/detail)',1),regexp_extract(url,'(video/openapi)',1),regexp_extract(url,'(user/profile)',1),regexp_extract(url,'(ugc/repost/)',1),regexp_extract(url,'(video/app)',1)) "+
                            "                when instr(url_host,'.snssdk.com')>0  and instr(url,'app_name=video_article')>0 then concat_ws('&',regexp_extract(url,'(app_name=video_article)',1),regexp_extract(url,'(vapp/action)',1),regexp_extract(url,'(vapp/danmaku)',1),regexp_extract(url,'(video/app/stream)',1),regexp_extract(url,'(keyword)',1),regexp_extract(url,'(category=xg_hotsoon_video)',1),regexp_extract(url,'(tt_shortvideo)',1),regexp_extract(url,'(category=tt_subv_live_tab)',1),regexp_extract(url,'(category=video_new)',1),regexp_extract(url,'(category=subv_user_follow)',1),regexp_extract(url,'(category=subv_xg_movie)',1),regexp_extract(url,'(category=subv_xg_game)',1),regexp_extract(url,'(category=subv_xg_society)',1),regexp_extract(url,'(category=subv_xg_nba)',1),regexp_extract(url,'(category=subv_xg_music)',1),regexp_extract(url,'(category=subv_variety)',1),regexp_extract(url,'(category=subv_video_agriculture)',1),regexp_extract(url,'(category=subv_video_food)',1),regexp_extract(url,'(category=subv_video_pet)',1),regexp_extract(url,'(category=subv_sport)',1),regexp_extract(url,'(category=subv_car)',1),regexp_extract(url,'(ugc/repost/)',1)) "+
                            "                when instr(url_host,'.snssdk.com')>0 and (instr(url,'app_name=live_stream')>0 or instr(url,'hotsoon')>0 or instr(url,'huoshan')>0) then concat_ws('&',regexp_extract(url,'(app_name=live_stream)',1),concat_ws('%',regexp_extract(url,'(hotsoon)',1),regexp_extract(url,'(query)',1)),regexp_extract(url,'(type=video)',1),regexp_extract(url,'(type=follow)',1),regexp_extract(url,'(room/enter)',1),regexp_extract(url,'(share/link_command)',1),regexp_extract(url,'(share_way=weixin)',1),regexp_extract(url,'(share_way=weibo)',1),regexp_extract(url,'(share_way=weixin_timeline)',1),regexp_extract(url,'(share_way=qq)',1),regexp_extract(url,'(share_way=qzone)',1),regexp_extract(url,'(comments)',1),concat_ws('%',regexp_extract(url,'(get_profile)',1),regexp_extract(url,'(to_user_id)',1)),regexp_extract(url,'(_follow)',1),regexp_extract(url,'(/ichat)',1),regexp_extract(url,'(_unfollow)',1),regexp_extract(url,'(get_notice)',1),regexp_extract(url,'(wallet)',1),regexp_extract(url,'(payment_channels)',1),regexp_extract(url,'(hashtag)',1),regexp_extract(url,'(karaoke_hot_videos)',1)) "+
                            "                when instr(url_host,'.snssdk.com')>0 and instr(url,'app_name=news_article_lite')>0 then concat_ws('&',regexp_extract(url,'(app_name=news_article_lite)',1),regexp_extract(url,'(category=.*?)&',1),regexp_extract(url,'(answer/detail)',1),regexp_extract(url,'(article/information)',1),regexp_extract(url,'(keyword)',1),regexp_extract(url,'(action=.*?)&',1),regexp_extract(url,'(video/play)',1),regexp_extract(url,'(from_category=.*?)&',1),regexp_extract(url,'(video/app)',1),regexp_extract(url,'(videolive)',1),regexp_extract(url,'(get_ugc_video)',1)) "+
                            "               else null " +
                            "          end urltag," +
                            "          pt_days,prov_id" +
                            "          from " + dataBase + ".bigflow_origin_sample_base a" +
    ...
    private void runActionAppnewAnalyzer(String ptDays,String dataBase) {
        ...
        runSparkJob(new CommonSparkJob(spark, logger, 1, dataBase, UnicomTable.USER_ACTION_APP_NEW_D, null, ptDays, null, null) {
            ...
            public Dataset<Row> run() throws UnsupportedEncodingException {
                ...
                Dataset<Row> originSampleBase = spark.sql("select a.pt_days, "+
                "       a.appname, "+
                "       a.device_number "+
                "   from    "+
                " USER_ACTION_APP_D a  "+
                ...
                saveData(appDay, defaultFormat, true, "pt_days");
                return null;
            }
        });
    }
    ...
    private void runActionApplossAnalyzer(String ptDays,String dataBase) {
        ...
        runSparkJob(new CommonSparkJob(spark, logger, 1, dataBase, UnicomTable.USER_ACTION_APP_LOSS_D, null, ptDays, null, null) {
            ...
            public Dataset<Row> run() throws UnsupportedEncodingException {
                ...
                Dataset<Row> originSampleBase = spark.sql("select '" + ptDays + "' pt_days, "+
                "       a.appname, "+
                "       a.device_number "+
                "   from    "+
                " USER_ACTION_APP_D a  "+
